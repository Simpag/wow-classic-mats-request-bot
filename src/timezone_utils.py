...
class WoWTimeHandler:
    def __init__(self, region: str = DEFAULT_WOW_REGION):
        ...
        if region not in WOW_TIMEZONES:
            raise ValueError(f"Invalid region. Must be one of: {list(WOW_TIMEZONES.keys())}")
    ...
    def format_wow_time(self, dt: Optional[datetime] = None, include_timezone: bool = True) -> str:
...
def format_for_discord(dt: Optional[datetime] = None, style: str = 'R', guild_id: Optional[int] = None) -> str:
...
def format_wow_time(dt: Optional[datetime] = None, guild_id: Optional[int] = None) -> str:
